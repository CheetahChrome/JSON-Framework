...
namespace JSON_Enumerate
{
    ...
    public static class JsonEnumerator
    {
                                                                                            ...
                                                                                            where P : IProperty, new()
        {
            ...
            switch (je.ValueKind)
            {
                ...
                case JsonValueKind.Array:
                    ...
                    if (arrItems.Any())
                    {
                        switch (arrItems.First().ValueKind)
                        {
                            case JsonValueKind.Array:
                            case JsonValueKind.Object:
                            ...
                            case JsonValueKind.Number:
                                if (instance.Properties == null)
                                    instance.Properties = new List<IProperty>();
                                instance.Properties.AddRange(
                                                      arrItems.Select(jp => WalkPropertiesWithNoKey<P, T>(jp, originParent))
                                                                                    .OfType<IProperty>());
        ...
        /// <summary>
        /// Handle the situation where an array contains no keys, just values.
        /// </summary>
        public static P WalkPropertiesWithNoKey<P, T>(JsonElement jElement, T originParent) where P : IProperty, new() where T : IJsonOperation, new()
        {
            //var key = jp.Name.Trim();
            var instance = new P() { // Name = key, 
                                     OverrideProperty = originParent.OverrideProperty };
